...
# Write a function to automate doublet removal workflow and update metadata
# Arg seurat_obj: pre-processed seurat object with cell clusters annotations
# Arg nrep: numeric; number of times to run the scDblFinder function
# Arg multisample_dataset: Logical indicating whether the processed dataset has multiple samples
# Arg sample_ids: if multisample_dataset = TRUE, a character that refers to the metadata column
# that contains sample ids
# Value: vector of consensus doublet cell barcodes
...
###########
# Write a function to standardize ambient mRNA contamination
# Arg seurat_obj: A seurat obj that has been filtered for doublets
# Value: A seurat object with a decontaminated raw counts matrix
...
# Write a function to standardize filtering and SCT normalization. We might need 
# to leave the FindClusters() function separate since that really depends on the
# number of cells on each dataset. 
# Arg Seurat obj: input seurat obj
# Arg seurat_filter: do we wish to filter?. Set to FALSE in pre-processing round for doublet removal
# Arg sample:  character indicating name of the sample
# Arg study: character indicating name of the study 
# Value: A seurat object that has been SCT normalized, nearest neighbors graph and UMAP embeddings
...
# Arg seurat obj: A seurat obj with reduced dims embedding like pca. Default now is set to PCA. 
# Arg clustering_res: A numeric vector with clustering resolutions to test
# Value: A data.frame with silhouette scores for a the provided clustering resolutions  
